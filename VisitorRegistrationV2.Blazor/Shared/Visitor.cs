...
namespace VisitorRegistrationV2.Blazor.Shared
{
    public class Visitor
    {
        ...
        public Task<bool> SetArrivalTime(DateTime? time, bool overRide)
            => ArrivalTime == null ? Task.FromResult((true, ArrivalTime = time).Item1)
                : (overRide == true ? Task.FromResult((true, ArrivalTime = time, DepartureTime = null).Item1) : Task.FromResult(false));
        ...
        public Task<bool> SetDepartureTime(DateTime? time, bool overRide)
            ...
            Task.FromResult((true, DepartureTime = time).Item1)
                : (overRide == true && ArrivalTime.HasValue ? Task.FromResult((true, DepartureTime = time).Item1) : Task.FromResult(false));
    }
}
